...
権限についての詳細は[権限の指定方法について](/permission)を確認していただければと思いますが、  
...
例：test01ファイルの権限にすべてのユーザーにすべての権限（777）を付与する場合
chmod 777 test01
...
```shell
例：testディレクトリのファイルから「test0」で始まるファイルの権限をすべてのユーザーに
　　すべての権限（777）に変更したい場合
chmod 777 test0*
...
例：test01ファイルの権限にファイルの所有者に対してすべての権限、それ以外には
　　読み込み権限（744）を付与する場合
chmod -v 744 test01
...
以下のコマンドで動きを試してみましょう。
```shell
例：testと名前の付くファイルおよびディレクトリにすべてのユーザーに
　　すべての権限（777）を付与する場合
chmod -R 777 test*
